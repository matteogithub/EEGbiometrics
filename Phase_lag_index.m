...
for i=1:N
    for j=1:N
        ...
        %PLI(i,j)=abs(mean(sign(sin(imag(complex_a(:,i)./complex_a(:,j)))))); %sin(imag(complex_a(:,i)./complex_a(:,j)))
        %PLI(i,j)=abs(mean(sign(imag(complex_a(:,i)./complex_a(:,j))))); %sin(imag(complex_a(:,i)./complex_a(:,j)))
        %PLI(i,j)=abs(mean(sign(angle(complex_a(:,i)./complex_a(:,j)))));
        PLI(i,j)=abs(mean(sign(sin(complex_aa(:,i)-complex_aa(:,j)))));
        %PLI(i,j)=abs(mean(sign(complex_aa(:,i)-complex_aa(:,j))));

        
        %uguali calcolando il sign      
        PLI(j,i)=PLI(i,j);     
        end
    end
end
